...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `nona()`.
	 *
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'nona-fonts', $google_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}

		// Enqueue Typekit Web Fonts.
		$typekit_fonts_url = 'https://use.typekit.net/dgq8tbc.css';
		if ( ! empty( $typekit_fonts_url ) ) {
			wp_enqueue_style( 'nona-typekit-fonts', $typekit_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = nona()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'nona-fonts', 'queue' ) ) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'nona' ), $handle ) ), 'Nona 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'nona_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! nona()->is_amp();
		 ...
		 */
		return apply_filters( 'nona_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = [
			...
			'nona-sidebar'    => [
				...
				'preload_callback' => function() {
					return nona()->is_primary_sidebar_active();
				},
			],
			'nona-widgets'    => [
		 ...
		 */
		$css_files = apply_filters( 'nona_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'nona_google_fonts', $google_fonts );
