...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			...
			[],
			nona()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'nona-navigation', 'async', true );
		wp_script_add_data( 'nona-navigation', 'precache', true );
		wp_localize_script(
			'nona-navigation',
			'nonaScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'nona' ),
				'collapse' => __( 'Collapse child menu', 'nona' ),
			]
		);
	}
